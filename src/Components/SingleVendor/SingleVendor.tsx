...
const SingleVendor = () => {
    ...
    // console.log(id);
    useEffect(() => {
        fetch(`https://blooming-chamber-05072.herokuapp.com/singleShop/${id}`)
            .then((res) => res.json())
            .then((data) => {
                setVendor(data);
            });
        fetch(`https://blooming-chamber-05072.herokuapp.com/singleShopProducts/${id}`)
            .then((res) => res.json())
            .then((data) => {
                setProducts(data);
                    ...
                    <div style={{}} className='row row-cols-lg-4 row-cols-sm-2 row-cols-md-3 '>
...

                        {
                            products.map((singleProduct) => {
                                if (singleProduct.isApproved) {
                                    return (<div key={singleProduct._id} className="mt-4 col">
